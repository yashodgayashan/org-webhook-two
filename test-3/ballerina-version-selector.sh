...
if [ ! -f  "${file_name}" ] ; then
  echo "BALLERINA TOML NOT EXISTS : SET BALLERINA DEFAULT VERSION (2201.6.0) " 
  ...
  if [ $((sanatized_ballerina_version)) -le 22010305 ]; then
    ...
    if [ $((sanatized_ballerina_version)) -gt 22010401 ] && [ $((sanatized_ballerina_version)) -le 22010500 ]; then
      ...
      # Check if the value is greater than or equal to 2201.5.0
      if [ $((sanatized_ballerina_version)) -gt 22010500 ]; then
